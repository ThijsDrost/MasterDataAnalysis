...
mask = (data[0].wavelength > wav_range[0]) & (data[0].wavelength < wav_range[1])
...
# %%
H2O2_wav = [260, 300]
H2O2_mask = (data[0].wavelength > H2O2_wav[0]) & (data[0].wavelength < H2O2_wav[1])

ref_H2O2_m = ref_H2O2[H2O2_mask]

def H2O2_model(wav, cH2O2):
    return cH2O2*ref_H2O2_m

H2O2_fit_model = lmfit.Model(H2O2_model)
H2O2_params = lmfit.Parameters()
H2O2_params.add('cH2O2', value=1, min=0)
...
H2O2_std = []
for index, measurement in enumerate(test):
    fit = H2O2_fit_model.fit(measurement[H2O2_mask], H2O2_params, wav=data[number].wavelength[H2O2_mask])

    # fit.plot()
    # plt.savefig(rf'{drive_letter()}:\OneDrive - TU Eindhoven\Master thesis\Plots\Calibration\NO2_H2O2\{index}.png')
    # plt.close()
