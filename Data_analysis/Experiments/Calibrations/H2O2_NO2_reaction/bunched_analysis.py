...
analyzer_H2O2 = CalibrationAnalyzer.standard(rf'{drive_letter()}:\OneDrive - TU Eindhoven\Master thesis\Measurements\Calibration\H2O2 cuvette\data.hdf5',
                                  'H2O2', 'H2O2 [mM]')
analyzer_NO3 = CalibrationAnalyzer.standard(rf'{drive_letter()}:\OneDrive - TU Eindhoven\Master thesis\Measurements\Calibration\NO3 cuvette\data.hdf5',
                                  'NO3-', 'NO3- [mM]')
analyzer_NO2 = CalibrationAnalyzer.standard(rf'{drive_letter()}:\OneDrive - TU Eindhoven\Master thesis\Measurements\Calibration\NO2 cuvette\data.hdf5',
                                  'NO2-', 'NO2- [mM]')
...
for i in range(len(data)):
    ...
    CalibrationAnalyzer.wavelength_range_ratio_vs_variable(ranges1, ranges2, variable_val_ticks=False)
    conc_lines = np.zeros(len(avg_inten))
    intensities = CalibrationAnalyzer.data_set.get_absorbances(masked=False)
