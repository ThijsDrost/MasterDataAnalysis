...
class CrossSectionData:
    ...
    def _write_comsol(self, file_handle):
        ...
        if self.process.lower() == 'excitation':
            ...
            file_handle.write(f' 12 1')
        file_handle.write('\n')
        file_handle.write(f'1.0 1.0\n')
    ...
    def check_data(self):
        ...
        for dat in self.data:
            ...
            if not self._check_derivative(dat.energy, dat.cross_section):
                p += f'Non-unique derivative found.\n'
            mask = self._select_spread(dat.energy)
            if not np.all(mask):
                p += f'Energy spread found.\n'
            if p != '':
...
def plot_CrossSections(data: list[CrossSectionData] | CrossSectionCollection, *, name_simplified=True, plot_kwargs=None, legend_kwargs=True, line_kwargs=None,
    ...
    if rotate_markers:
        for index in range(len(data)):
            line_kwargs_iter[index] = Plot.set_defaults(line_kwargs_iter[index], **{'marker': Plot.marker_cycler(index)})
    if color_by == 'process':
        color_by = [dat.process for dat in data]
        color_line_kwargs_iter = Plot.linelook_by(color_by, colors=True)
    ...
    return Plot.lines(x_values, y_values, labels=labels, plot_kwargs=plot_kwargs, legend_kwargs=legend_kwargs, line_kwargs=line_kwargs, show=show,
                      line_kwargs_iter=line_kwargs_iter, **kwargs)


def plot_averageCrossSections(data: CrossSectionCollection, *, energy_kwargs=None, average_kwargs=None, line_kwargs_avg=None, line_kwargs=None, fig_ax=None,
...
def plot_spread(data: list[CrossSectionData] | CrossSectionCollection, bins=10, *, histtype='step', show=True, close=False, log=False):
    if len(data) > 10:
        warnings.warn(f'More than 10 datasets are being plotted, not enough unique colors are available.')
    fig, ax = plt.subplots()
    for dat in data:
