...
from General.experiments.spectrum import TemporalSpectrum, Spectrum


@attrs.define(frozen=True)
class TemporalSpectroData:
    """
    Class for storing temporal spectral data

    Parameters
    ----------
    spectrum: TemporalSpectrum
        The temporal spectral data
    serial_number : str
        The serial number of the spectrometer
    integration_time_ms : float
        The integration time in milliseconds
    n_averages : int
        The number of averages per measurement
    n_smoothing : int
        The number of smoothing points
    start_time_s : int
        The time of the first measurement in seconds since epoch
    """
    spectrum: TemporalSpectrum
    serial_number: str
    integration_time_ms: float
    ...
    def read_folder(filename):
        ...
        if not all([read_files[0].same_measurement(d) for d in read_files[1:]]):
            raise ValueError(f'Not all files in {filename} contain the same measurement.\nDifference:'
                             f' {read_files[0].give_diff(read_files[1])}')
        times = np.array([d.time_ms for d in read_files])
        times = times - times[0]
        intensities = np.array([d.spectrum.intensities for d in read_files]).T
        return TemporalSpectroData(TemporalSpectrum(read_files[0].spectrum.wavelengths, intensities, times),
                                   read_files[0].serial_number, read_files[0].integration_time_ms, read_files[0].n_averages,
                                   read_files[0].n_smoothing, read_files[0].time_ms / 1000)
    ...
    def get_intensity(self, multi_method='mean'):
        if multi_method in ('mean', 'average'):
            ...
            return np.median(self.spectrum.intensities, axis=1)
...
class SpectroData:
    """
    Class for storing spectral data
    ...
    n_smoothing : int
        The number of smoothing points
    time_ms : int
        The time of the measurement in milliseconds since epoch
    relative_times_ms : np.ndarray = None
        Relative times in milliseconds, if the data is from a folder with multiple measurements
    """
    spectrum: Spectrum
    serial_number: str
    integration_time_ms: float
    ...
    def same_measurement(self, measurement):
        if not isinstance(measurement, SpectroData):
            raise TypeError(f'Can only compare SpectroData to SpectroData, not {type(measurement)}.')
        ...
        if not (self.n_smoothing == measurement.n_smoothing):
            return False
        if not np.all(np.isclose(self.spectrum.wavelengths, measurement.spectrum.wavelengths, rtol=1e-3)):
            return False
        return True

    def give_diff(self, other: SpectroData):
        message = ""
        ...
        if not np.all(np.isclose(self.spectrum.wavelengths, other.spectrum.wavelengths, rtol=1e-3)):
            message += "wavelengths are different\n"
    ...
    def get_intensity(self, multi_method='mean'):
        if self.spectrum.intensities.ndim == 1:
            return self.spectrum.intensities
        if self.spectrum.shape[1] == 1:
            return self.spectrum.intensities[:, 0]
        if multi_method in ('mean', 'average'):
            return np.mean(self.spectrum.intensities, axis=1)
        elif multi_method in ('sum', 'total'):
            return np.sum(self.spectrum.intensities, axis=1)
    ...
    def read_txt(filename) -> SpectroData:
        ...
        read_lines = open(filename, 'r').readlines()
    ...
    def _read_ava_multi_txt(lines: list[str], filename):
        ...
        return SpectroData(Spectrum(spectra.index, spectra.to_numpy()), serial_number, integration_time, file.shape[1], n_smoothing, timestamp_ms, timestamps_ms)
    ...
    def _read_ava_single_txt(lines: list[str], filename):
        ...
        return SpectroData(Spectrum(wavelength, intensity), serial_number, integration_time, n_averages, n_smoothing, timestamp_ms)
    ...
    def read_raw8(filename):
        ...
        spectrum = Spectrum(h['wavelength'], h['intensity'])
        del h['wavelength'], h['intensity']
        return SpectroData(spectrum=spectrum, **h)
