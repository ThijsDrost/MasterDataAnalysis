...
from General.experiments.absorption.DataSets import DataSet, import_hdf5
...
class MeasurementsAnalyzer:
    ...
    def _averages(values, n_averages: int, average_over: int):
        if n_averages * average_over > len(values):
            warnings.warn('`average_over`*`n_averages` > `len(values)`, the averaging blocks will overlap')
    ...
    def _avg(x, y, num, average_num):
        ...
        if isinstance(num, int):
            if average_num is None:
                ...
                x = block_average(x, average_num)
                y = block_average(y, average_num)
            ...
            x = block_average(x, num_val)
            y = block_average(y, num_val)
        else:
    ...
    def absorbance_with_wavelength_over_time(self, num, timescale='min', average_num: int = None, *, corrected=True, masked=True, plot_kwargs=None, save_loc=None, save_suffix='', show=True, save_kwargs=None,
        ...
        plot_kwargs = plot.set_defaults(plot_kwargs, xlabel='Wavelength [nm]', ylabel='Absorbance [A.U.]', xlim=(xs[0], xs[-1]))
    ...
    def absorbance_over_time_with_wavelength(self, num, timescale='min', average_num: int = None, time_num: int = None, norm=True, *,
        ...
        sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
        cbar_kwargs = plot.set_defaults(cbar_kwargs, label='Wavelength [nm]', mappable=sm, ticks=ticks)
        line_kwargs = plot.set_defaults(line_kwargs)
    ...
    def total_absorbance_over_time(self, num, timescale='min', average_num: int = None, wav_range=None, *, corrected=True, masked=True,
        ...
        xs, ys = self._avg(xs, ys, num, average_num)
        plot_kwargs = plot.set_defaults(plot_kwargs, xlabel=f'Time [{timescale}]', ylabel='Total absorbance [A.U.]')
    ...
    def fit(self, model: lmfit.CompositeModel, timescale='min', num=None, average_num=None, use_previous=True, wavelength_range: tuple[float, float] = None, *,
        ...
        if np.isnan(xs).any():
            raise ValueError(f'NaN values in xs')
        for i, y in enumerate(ys):
            if np.isnan(y).any():
                warnings.warn(f'NaN values in y at index {i}')
                results.append(None)
                continue
        ...
        names = []
        for param in params:
            res.append(np.array([result.params[param].value if result else np.nan for result in results]))
            std.append(np.array([result.params[param].stderr if result else np.nan for result in results]))
            names.append(param)
