...
class DataSet(SimpleDataSet):
    ...
    def remove_index(self, index):
        mask = np.ones(len(self.variable), dtype=bool)
        mask[index] = False
        return DataSet(self.wavelength, self.absorbances[mask], self.variable[mask], self.measurement_num[mask],
                       self.variable_name, self.wavelength_range, self._selected_num, self.baseline_correction)
