...
def read_folder(loc, hdf5_place, notes, pH=None, conductivity=None, species_dicts=None, find_method='closest_before',
                spectrum_type='absorbance', multi_methods: dict = None):
    ...
    hdf5_place.attrs.create('wavelength', temp_read.spectrum.wavelengths)
    ...
    for file in os.scandir(loc):
        ...
        if data.spectrum.intensities.ndim == 2:
        ...
        if data.spectrum.intensities.ndim == 1:
            dataset.attrs.create('averaging', data.n_averages)
        else:
            dataset.attrs.create('averaging', data.spectrum.intensities.shape[1])
