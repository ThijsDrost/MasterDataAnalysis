...
class CalibrationAnalyzer:
    ...
    def pearson_r_vs_wavelength(self, *, corrected=True, masked=True, num='plot', save_loc=None, save_suffix='',
                                plot_kwargs=None, show=False, line_kwargs=None, save_kwargs=None, **kwargs):
        linearity = np.zeros(len(self.data_set.get_wavelength(masked)))
        for i in range(len(self.data_set.get_wavelength(masked))):
            linearity[i] = scipy.stats.pearsonr(self.data_set.variable_num, self.data_set.get_absorbances(corrected=corrected, masked=masked, num=num)[:, i])[0]
    ...
    def one_minus_pearson_r_vs_wavelength(self, *, corrected=True, masked=True, num='plot', save_loc=None, save_suffix='',
                                plot_kwargs=None, show=False, line_kwargs=None, save_kwargs=None, **kwargs):
        linearity = np.zeros(len(self.data_set.get_wavelength(masked)))
        for i in range(len(self.data_set.get_wavelength(masked))):
            linearity[i] = 1-scipy.stats.pearsonr(self.data_set.variable_num, self.data_set.get_absorbances(corrected=corrected, masked=masked, num=num)[:, i])[0]
