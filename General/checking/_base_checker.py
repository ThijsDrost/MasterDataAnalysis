...
from General.checking import NoValue
from General.checking.number_line import NumberLine
from General.checking import ValidatorError
...
class BaseChecker:
    ...
    def numpy(cls, dims: int, shape: int | tuple[int], dtype: type, **kwargs) -> Self:
        ...
        dtype: type
            The correct dtype
        """
        def check_numpy(dims, shap, dtype):
            def checker(value):
                if value.ndim != dims:
                    return ValueError(f"Value must have {dims} dimensions, not {value.ndim}")
                if isinstance(shap, int):
                    shap = (shap,)
                if value.shape != shap:
                    return ValueError(f"Value must have shape {shap}, not {value.shape}")
                if value.dtype != dtype:
                    return ValueError(f"Value must have dtype {dtype}, not {value.dtype}")
            return checker
